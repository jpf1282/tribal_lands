...
protect_area_0 <- read_csv("_Variables/protected_areas_master.csv", col_types = "cddddddd") # Use d for double as it contains some scientific notation cells
sheldus_0 <- read_csv("_Variables/SHELDUS/SHELDUS_to_merge_final.csv", col_types = "cdddddddddd") %>%
...
extra_dedup <- full_join(amenity_dedup, oil_gas_dedup, by = c("FIPS_r", "FIPS")) %>%
  ...
  full_join(sheldus_dedup, by = c("FIPS_r")) %>% ## CHECK ## Only join on FIPS_r because it is already all 5 digits with starting 0
  select(-FIPS.y) %>%
  rename(FIPS = FIPS.x)
...
merged_data_record_all_t1 <- select(merged_data_record_all, ID, tribe, FIPS_t1, n_unique_FIPS_t1, amen_rank_t1, oil_avg_t1, gas_avg_t1, precip_t1, Risk_t1, Governance_t1, `Built Environment_t1`, `Natural Environment_t1`, Society_t1, CRSI_t1, p_all_t1, p_blm_t1, p_dod_t1, p_doe_t1, p_nps_t1, p_usfs_t1, p_usfws_t1, shel_crop_t1,
shel_crop_pc_t1, shel_prop_t1, shel_prop_pc_t1, shel_injur_t1, shel_injur_pc_t1,shel_fatal_t1, shel_fatal_pc_t1, shel_dura_t1, shel_yr_total_t1, lon_t1, lat_t1, avg_lat_t1, avg_lon_t1, mid_lat_t1, mid_lon_t1) %>% 
  mutate(time = "time 1") %>%
  rename(FIPS = FIPS_t1,
...
merged_data_record_all_t2 <- select(merged_data_record_all, ID, tribe, FIPS_t2, n_unique_FIPS_t2, amen_rank_t2, oil_avg_t2, gas_avg_t2, precip_t2,  Risk_t2, Governance_t2, `Built Environment_t2`, `Natural Environment_t2`, Society_t2, CRSI_t2, p_all_t2, p_blm_t2, p_dod_t2, p_doe_t2, p_nps_t2, p_usfs_t2, p_usfws_t2, shel_crop_t2,
shel_crop_pc_t2, shel_prop_t2, shel_prop_pc_t2, shel_injur_t2, shel_injur_pc_t2,shel_fatal_t2, shel_fatal_pc_t2, shel_dura_t2, shel_yr_total_t2, lon_t2, lat_t2, avg_lat_t2, avg_lon_t2, mid_lat_t2, mid_lon_t2) %>% 
  mutate(time = "time 2") %>%
  rename(FIPS = FIPS_t2,
