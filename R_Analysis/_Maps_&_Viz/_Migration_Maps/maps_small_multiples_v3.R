...
#                       ) #remove title
#       #background plot
#       usa<-ggplot()+geom_polygon(data=county_map, aes(x=cm_long, y=cm_lat,group=group),fill="black",colour="darkgrey",size=.06)+
#        theme_bw()+maptheme 
#       
...
#       #commented out the original below
#       #add state outline
#       usa_tribe_states<-usa+geom_polygon(data=states_df, aes(x=cm_long, y=cm_lat,group=group),fill="black",colour="darkgrey",size=.06)
#       
#       #add state outline
...
#       tribe_df_map<-usa_tribe_states+
#         geom_polygon(data=jf_map, aes(x=cm_long, y=cm_lat, fill=time))+  #,group=group
#         zoom[[z]]+ggtitle(t)
#       
#       timept<-unique(jf_map$time)
#      
#         if(("time1" %in% timept) & ("time2" %in% timept)){
#           tribe_df_map<-tribe_df_map+scale_fill_manual(values=c("#ffff00","#ff0000"),labels=c(" Historical Areas ", " Present-day Areas" ),name="Time")
#           
#         } else if (("time1" %in% timept) & !("time2" %in% timept)){
#           tribe_df_map<-tribe_df_map+scale_fill_manual(values=c("#ffff00"),labels=c(" Historical Areas "),name=" Present-day Areas ")
#          
#         } else if (!("time1" %in% timept) & ("time2" %in% timept)){
#           tribe_df_map<-tribe_df_map+scale_fill_manual(values=c("#ff0000"),labels=c(" Historical Areas "),name=" Present-day Areas ")
#          
#         }
# 
#       # add migration lines
...
#     
#       
#        tribe_df_mig$label<-"Migration Flows from Historical to Present-day" #COMMENTED THIS OUT BECAUSE IT WONT WORK IF I USE CUSTOM MIGRATION LINE COLOR LINES
#        tribe_df_map <- tribe_df_map + geom_curve(data = tribe_df_mig,aes(x = l_lon1, y = l_lat1, xend = l_lon2, yend = l_lat2, color=label),
#                                                   alpha=opacity,size = .3,curvature = -0.4,arrow = arrow(type="closed",angle=5,length = unit(0, "inches")))+
...
#        
#         rows<-ntribes/4 # I EDITED THIS SO THAT THERE ARE TWO PER... change back to 4 if need be
#         extra<-ntribes%%4  # I EDITED THIS SO THAT THERE ARE TWO PER... change back to 4 if need be
#         rows<-ifelse(extra>0, ceiling(rows)+1,rows+1)
#         if(!legend_found){lgnd<-get_legend(tribe_df_map)}
#         relh[1:rows]<-1
#         relh[length(relh)]<-.5
#         cols<-ifelse(ntribes<4, ntribes,4)  # I CHANGED this to ",2" THIS SO THAT THERE ARE TWO PER... change back to 4 if need be
#       } 
#       
#       
#         
