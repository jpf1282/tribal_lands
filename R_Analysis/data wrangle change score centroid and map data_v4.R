...
# add '_Variables' to import path below, then add path to spatial files
...
extra_dedup <- full_join(amenity_dedup, oil_gas_dedup, by = c("FIPS_r", "FIPS")) %>%
  ...
  full_join(protect_area_dedup, by = c("FIPS_r", "FIPS"))
...
merged_data_record_t1 <- select(merged_data_record, tribe, FIPS_t1, n_unique_FIPS_t1, amen_rank_t1, oil_avg_t1, gas_avg_t1, precip_t1, Risk_t1, Governance_t1, `Built Environment_t1`, `Natural Environment_t1`, Society_t1, CRSI_t1, p_all_t1, p_blm_t1, p_dod_t1, p_doe_t1, p_nps_t1, p_usfs_t1, p_usfws_t1, lon_t1, lat_t1, avg_lat_t1, avg_lon_t1, mid_lat_t1, mid_lon_t1) %>% 
  mutate(time = "time 1") %>%
  rename(FIPS = FIPS_t1,
         n_unique_FIPS = n_unique_FIPS_t1,
         amen_rank = amen_rank_t1,
...
merged_data_record_t2 <- select(merged_data_record, tribe, FIPS_t2, n_unique_FIPS_t2, amen_rank_t2, oil_avg_t2, gas_avg_t2, precip_t2, Risk_t2, Governance_t2, `Built Environment_t2`, `Natural Environment_t2`, Society_t2, CRSI_t2, p_all_t2, p_blm_t2, p_dod_t2, p_doe_t2, p_nps_t2, p_usfs_t2, p_usfws_t2, lon_t2, lat_t2, avg_lat_t2, avg_lon_t2, mid_lat_t2, mid_lon_t2) %>% 
  mutate(time = "time 2") %>%
  rename(FIPS = FIPS_t2,
         n_unique_FIPS = n_unique_FIPS_t2,
         amen_rank = amen_rank_t2,
...
merged_data_tr1 <- select(merged_data_tr1_r2, tribe, ID, avg_amen_rank, avg_oil, avg_gas, avg_precip, avg_risk, avg_governance, avg_built_env, avg_natural_env, avg_society, avg_crsi, avg_p_all, avg_p_blm, avg_p_dod, avg_p_doe, avg_p_nps, avg_p_usfs, avg_p_usfws, ends_with("_t1")) %>%
  rename(
...
merged_data_r2 <- select(merged_data_tr1_r2, tribe, ID, FIPS, time, amen_rank, oil_avg, gas_avg, precip, risk, governance, built_env, natural_env, society, crsi, p_all, p_blm, p_dod, p_doe, p_nps, p_usfs, p_usfws, lon, lat, ends_with("_t2"))
