---
title: "Explore Difference in Precipitation"
...
* The distribution of precipitation in time 1 and time 2 is slightly skewed to the right.
* The distirbution of precipitation is in general lower in time 2 compared to time 1.

```{r, message = FALSE, warning = FALSE}

ggplot(data_long, aes(precip, fill = time)) +
...
* Use Welch's t-test because the outcome precipitation is a continous measures.
...
ttest_model <- t.test(precip ~ time, data = data_long, paired = FALSE)
...
mean_t1 <- round(mean(filter(data_long, time == "time 1")$precip, na.rm = T), 2)
mean_t2 <- round(mean(filter(data_long, time == "time 2")$precip, na.rm = T), 2)
sd_t1 <- round(sd(filter(data_long, time == "time 1")$precip, na.rm = T), 2)
sd_t2 <- round(sd(filter(data_long, time == "time 2")$precip, na.rm = T), 2)
...
* At the aggregate level,  the difference in precipitation between time 1 (M = `r mean_t1`, SD = `r sd_t1`) and time 2 (M = `r mean_t2`, SD = `r sd_t2`) was not statistically signficant (t(`r degreef`)= `r tvalue`, `r ifelse(pvalue < .001, "p < .001", paste0("p = ", pvalue))`).

### Use only records where tribes have time 1 and 2 data

```{r}

ttest_model <- t.test(precip ~ time, data = data_t1and2_long, paired = FALSE)
...
mean_t1 <- round(mean(filter(data_t1and2_long, time == "time 1")$precip, na.rm = T), 2)
mean_t2 <- round(mean(filter(data_t1and2_long, time == "time 2")$precip, na.rm = T), 2)
sd_t1 <- round(sd(filter(data_t1and2_long, time == "time 1")$precip, na.rm = T), 2)
sd_t2 <- round(sd(filter(data_t1and2_long, time == "time 2")$precip, na.rm = T), 2)
...
* At the aggregate level for tribes with migration, the difference in precipitation between time 1 (M = `r mean_t1`, SD = `r sd_t1`) and time 2 (M = `r mean_t2`, SD = `r sd_t2`) was also not statistically signficant (t(`r degreef`)= `r tvalue`, `r ifelse(pvalue < .001, "p < .001", paste0("p = ", pvalue))`).
...
precip_data_wide <- select(data_wide, ID, tribe, FIPS_t1, FIPS_t2, time, precip_t1, precip_t2, precip_change_score) %>%
  filter(!is.na(precip_t1), !is.na(precip_t2))

precip_data_long <- gather(precip_data_wide, "time", "precip", -ID, -tribe, -time, -precip_change_score, -FIPS_t1, -FIPS_t2) %>% ungroup() %>%
...
* Use Repeated Measure ANOVA because the outcome precipitation is a continous measures and is predicted by a single within subject factor (time).
* Repeated Measure ANOVA can only be run with tribes with both time 1 and time 2 data.

###  Use only records where tribes have time 1 and 2 data
```{r}

aov_model <- aov(precip ~ time + Error(tribe / time), data = precip_data_long)

summary(aov_model)

lm_model <- lm(precip ~ time , 
               weights = wgt,
               data = precip_data_long %>%
                 group_by(tribe) %>%
                 mutate(wgt=1/n()) %>%
                 ungroup())
library(sandwich)
...
* At the aggregate level for tribes with migration, the difference in precipitation between time 1 (M = `r mean_t1` SD = `r sd_t1`) and time 2 (M = `r mean_t2`, SD = `r sd_t2`) was statistically signficant (F(`r degreef1`, `r degreef2`)= `r Fvalue`, `r ifelse(pvalue < .001, "p < .001", paste0("p = ", pvalue))`).
...
do(tryCatch( {tidy(t.test(precip ~ time, data = ., paired = FALSE))},
  error = function(e) {tidy(t.test(precip ~ time, data = data_t1and2_long)) %>% mutate(estimate = NA)}
    ))
...
This can only be tested with the `r nrow(tribes_time1and2_lst)` tribes where migrations occured. Furthermore, only tribes where precipitation have changed between time points and with more than one data point at each time point can their differences be tested for statistical significance. This reduce the number of tribes from `r nrow(tribes_time1and2_lst)` to `r no_test`.
...
Calculate number of test based only on tribes where there is non-zero difference and sufficient data point in precipitation between time 1 and 2.
...
* Add average precipitation, distance, and no of unique FIPS to help with interpretation
* Rename some variables for prettier column headers

```{r}

results_df <- inner_join(model, merged_data_tribe, by = "tribe") %>%
  mutate(`Avg Diff Precipitation` = avg_precip_t2 - avg_precip_t1) %>%
  select(tribe, sig, p.value, `Avg Diff Precipitation`, avg_precip_t1, avg_precip_t2, n_unique_FIPS_t1, n_unique_FIPS_t2, mid_dist) %>%
  arrange(p.value) %>%
  rename(
    `Avg Precipitation T1` = avg_precip_t1,
    `Avg Precipitation T2` = avg_precip_t2,
...
datatable(results_df,
   ...
   formatRound(columns = c('Avg Diff Precipitation', 'Avg Precipitation T1', 'Avg Precipitation T2', 'Mid Distance'), digits = 2)
...
* The mean precipitation comparisons for `r nrow(b_sig_tribe_lst)` tribes (`r b_sig_tribe_lst$tribe`) achieve statistical significance when the Bonferroni correction is used.
...
* If Bonferroni correction is not used, precipitation was significantly differ between time 1 and 2 for `r nrow(sig_tribe_lst)` tribes :(`r sig_tribe_lst$tribe`).
...
* Overall, no statistically significant difference was found at the aggregate level. At the tribe level, when correction for multiple comparisons was made, precipitations for `r nrow(b_sig_tribe_lst)` tribes were found to be statistically differ between time 1 and time 2. 
...
model <- group_by(precip_data_long, tribe) %>%
do(tryCatch( {tidy(aov(precip ~ time + Error(ID / time), data = .))},
  error = function(e) {tidy((aov(precip ~ time + Error(ID / time), data = .))) %>% mutate(estimate = NA)}
    ))
...
Calculate number of test based only on tribes where there is non-zero difference in percipitation between time 1 and 2.
...
* Overall, a statistically significant difference was found at the aggregate level. At the tribe level, when correction for multiple comparisons was made, precipitation for `r nrow(b_sig_tribe_lst)` tribes were found to be statistically differ between time 1 and time 2. Without correction, precipitation was found to be statistically differ for `r nrow(sig_tribe_lst)` tribes.
