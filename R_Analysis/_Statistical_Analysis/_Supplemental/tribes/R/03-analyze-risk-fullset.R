...
#' The left tail is fatter than the right tail, and this results a slightly
#' negative sample average. It is unclear from these plots that if the 
#' mean `r target` change from t1 to t2 is significantly different from zero.
...
plot_t1t2(df_long, use_notch = T, use_scale = "pct", fig_ylab = target)
...
#' According to the ANOVA output, `time` is significant with a p-value of 0.00381. 
...
#' For the mean values, we obtained a r-squared value of 0.25, which translates
#' to a correlation of 0.5 (the squared root of 0.25). The slope of the line is 
#' 0.79, meaning that for every 100% increase in risk at t1, 
#' we can expect a 79% increase at t2. This is statistically signicant by 
#' the tiny p-value. A similar interpretation can be done for the median values.

plot_t1_vs_t2(df, use_scale = "pct", fig_tit = target)