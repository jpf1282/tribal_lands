...
t.test(oil_avg ~ time, data = data_long, paired = FALSE)
...
pvalue <- round(tidy(t.test(oil_avg ~ time, data = data_long, paired = FALSE))$p.value, 3)
tvalue <- round(tidy(t.test(oil_avg ~ time, data = data_long, paired = FALSE))$statistic, 3)
degreef <- round(tidy(t.test(oil_avg ~ time, data = data_long, paired = FALSE))$parameter, 3)
...
```{r}
t.test(oil_avg ~ time, data = data_t1and2_long, paired = FALSE)
```
...
pvalue <- round(tidy(t.test(oil_avg ~ time, data = data_t1and2_long, paired = FALSE))$p.value, 3)
tvalue <- round(tidy(t.test(oil_avg ~ time, data = data_t1and2_long, paired = FALSE))$statistic, 3)
degreef <- round(tidy(t.test(oil_avg ~ time, data = data_t1and2_long, paired = FALSE))$parameter, 3)
...
* The mean oil production comparisons for one tribe (`r b_sig_tribe_lst$tribe`) achieve statistical significance when the Bonferroni correction is used.
...
* If Bonferroni correction is not used, oil production was significantly differ between time 1 and 2 for eight tribes :(`r sig_tribe_lst$tribe`).
...
* Overall, a statistically significant difference was found at the aggregate level when looking at all tribes data, not just those with migrations. At the tribe level, when correction for multiple comparisons was made, oil production for one tribe were found to be statistically differ between time 1 and time 2. Without correction, oil production was found to be statistically differ for 7 tribes. 
