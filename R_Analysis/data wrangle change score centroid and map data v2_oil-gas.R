...
oil_gas_0 <- read_csv("_Variables/Oil_Gas_Minerals/oilgascounty_avg.csv", col_types = "cccciiiiii")
...
# Don't need to check amenity against oil and gas here
...
merged_data <- mutate(merged_data,
  ...
  oil_change_score = oil_avg_t1 - oil_avg_t2,
  gas_change_score = gas_avg_t1 - gas_avg_t2)

# Calculate tribe level change scores
merged_data <- mutate(merged_data,
  avg_amenity_change_score = avg_amen_rank_t1 - avg_amen_rank_t2,
  avg_oil_change_score = avg_oil_avg_t1 - avg_oil_avg_t2,
  avg_gas_change_score = avg_gas_avg_t1 - avg_gas_avg_t2)
...
merged_data_meta <- select(merged_data_tribe,
    ...
    -avg_oil_avg_t1, -avg_oil_avg_t2,
    -avg_gas_avg_t1, -avg_gas_avg_t2,
